...
- **F-031 (P1, new; partially contained same evening):** backup passphrase (65B, sole copy, workspace/secrets) had zero escrow/off-host copy → host loss = all offsite artifacts undecryptable. **~19:16 PDT, owner-directed (QC3B-11): rotated to an owner-chosen passphrase** — old key preserved at `resort-backup-passphrase.pre-rotation-20260815` (never delete), old key proven to decrypt the archive, new key proven end-to-end (forced backup → offsite upload → drill decrypt, `crm-2026-08-16T021807Z`). **Key lineage: artifacts ≤08-15T213917Z need the pre-rotation key; ≥08-16T021807Z the current one.** Remaining: off-host escrow of the OLD key (guards all 118 pre-rotation artifacts); caveats recorded (new value passed through the operator transcript; short human-pattern vs 64-char random).
...
None for Monday's drill except the clock (D-006 window). Open D-rows: D-004, D-007 (validator), D-009, D-010 (confirm), D-001 remaining rows, D-002 owner-cash-flow row; **D-006 recorded** — owner may correct the conservative window interpretation and answer the remaining attestation questions (**F-031 old-key off-host escrow — the one line still standing after the rotation**; F-032 media offline copies; warmup accepted-loss disposition). Open P1s: F-001 (QC-6), F-005 (QC-7 gate), F-014 (QC-4), F-015 + F-016 (fix session per D-008), **F-031 (partially contained — old-key escrow outstanding), F-032 (non-CRM coverage)**. Open P2s: F-023, F-025 (corrected/rescoped), F-006, F-029, **F-033**. Open P3s: F-024, F-026, F-021, F-027, F-028, F-030, **F-034**.
...
2. Drill (FIXTURE, scratch-only, per D-006): on the two already-retrieved offsite artifacts in `~/qc-evidence/QC3B/offsite/` — time decrypt **using the PRE-ROTATION key file** (`…/resort-backup-passphrase.pre-rotation-20260815` — both artifacts predate the QC3B-11 rotation) → gunzip → quick_check → QC3A oracle probes for BOTH `crm-2026-08-15T213917Z` (lineage-comparable) and `crm-2026-08-14` daily (offsite-only artifact); optionally repeat on the offsite `crm-2026-08-16T021807Z` with the CURRENT key for new-lineage proof; record end-to-end off-host RTO (retrieval was seconds on 2026-08-15) vs ≤4h target; expect migration-009-style guards N/A (no migrations run — restore+verify only).
3. Record QC3B-12 (+ QC3B-13 if RTO row split); mark F-018 fully closed, update F-013; FINDINGS/RISK_HYPOTHESES/STATUS updates.
