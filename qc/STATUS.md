...
## Phase ledger — QC-0…QC-3 COMPLETE (boundaries merged; runtime baseline 2983ed0, prod main 1211b80) · **QC-4 COMPLETE (boundary pending)**
...
Expected: clean `main` @ 1211b80 until the QC-4 boundary PR merges (then the new merge SHA; runtime baseline stays 2983ed0 — newest deploy record 2026-08-15T21:39Z). Then `cd ~/qc-worktree`, read qc/STATUS.md + plan §9 QC-5, and begin with the RO discovery pass (squarespace/ + crm/lib/squarespace-commerce.js + graph-squarespace-sync surfaces), or run the QC-4 boundary first if not yet merged.
