...
const WORKFLOW_SCHEMA = {
  ...
  properties: {
    workflow: {
      ...
      description: 'Versioned workflow to execute. Read results are returned in this tool content; never claim a controlled channel hides them or redirect the user to another channel. Use crm.contacts.read for contact or POC lookups in #whatsapp; do not defer those requests to another channel. ownerrez.mutation.propose accepts any operation in the fixed OwnerRez mutation catalog (see the input guidance); the server validates the operation id, so never tell a user an operation is outside an allowed set unless this tool returned that rejection.',
    },
    input: {
      type: 'object',
      description: 'Workflow input. For receipts.status.read use query, date/start/end, amount, currency, status or scope (all/reconciled/pending), order, and limit to return actual receipt/QBO rows. For accounting.reconciliation.read use view=summary or transactions and order=asc or desc. For crm.contacts.read use query for one contact, queries for multiple names/numbers/emails, and optional limit (1-100); it returns authorized full CRM contact details. For whatsapp.status.read use direction (outbound by default, inbound, or all), optional limit (1-100), and optional messageSid; a trusted current Slack thread is applied automatically. For ownerrez.mutation.propose pass operationId (an exact id from the fixed OwnerRez v2 mutation catalog in crm/lib/ownerrez-mutation-catalog.js, for example TagDefinitions_Post, Bookings_Patch, Guests_Delete), pathParams for the {id}-style fields of the operation path, query only where the operation declares query parameters, body for POST/PATCH operations, and reason (4-500 characters); every catalog operation may be proposed, nothing changes in OwnerRez until the same user types the emitted !ownerrez confirm command within 15 minutes, and the reservations command reference lists every operation id. WhatsApp sends are command-only and cannot be invoked through this tool.',
