...
| F-018 | Backup path may diverge from CRM path; offsite retrieval untested; non-CRM state (Paloma tasks.db, policy, registry) lacks demonstrated coverage | QC-3. **Partially validated 2026-08-15 (QC-3a):** the path-divergence *mechanism* is real and recorded under F-006 (P2); backup SOURCE currently identical to runtime DB by default; `crm_backup.py` uses the proper online-backup API from a `mode=ro` handle + quick_check + 600. Offsite retrieval, retention, restore drills, and non-CRM coverage remain for QC-3b (D-006 gate) |
