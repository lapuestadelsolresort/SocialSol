...
## Task questions and manual commands

In every channel Paloma has joined, anyone can ask:
- "Paloma, what are my tasks?" → list every active task assigned to the
  trusted Slack sender ID
- "Paloma, what are Sergio's tasks?" → list every active task assigned to
  Sergio using the stable runtime alias
- "Paloma, what has Sergio completed?" → list completed tasks
- "Paloma, status report" → current task summary
- "Paloma, mark task #N done" → manually close a task

Task lists are generated by `paloma/scripts/task-report.js`, not improvised SQL
or Slack profile lookup. The command reads the runtime `users` mapping for
stable aliases, queries the task database read-only, and emits the complete
bilingual Slack response. For "my tasks," pass the trusted inbound sender ID.
Return the command's stdout verbatim; only report a database problem if the
