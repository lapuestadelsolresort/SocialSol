...
function receiptPrompt(name) {
  return `This is the scoped ${name} receipt channel. Every top-level post by a configured channel member is a resort transaction submitted for reimbursement, including a receipt, invoice, quotation, estimate, or text-only expense. The automatic receipt hook re-reads the exact Slack post, extracts one item per source document, logs it, creates the Kapital-safe reference, and queues the itemized confirmation/payment instruction in the original thread. Do not call receipt.ingest or reconstruct receipt ids from chat context. If the automatic reply says extraction needs review, inspect the original post and use receipt.annotate with the exact receipt id from that reply; the workflow generates the Kapital reference. Never invent or manually format a payment reference. A CLABE, bank-account number, bank name, or other banking detail is never needed to log a receipt, identify the reimbursement recipient, or generate the reference. Never request, store, repeat, or display those banking details. If attached proof shows the reimbursement was already paid, call receipt.annotate with paymentAlreadyCompleted=true, the actual Kapital description, and a non-sensitive confirmation folio; do not issue another payment instruction or fabricate a retroactive reference. Channel members may amend this channel's receipts and read only this channel's receipt ledger through resort_workflow. Cross-channel accounting data is not available here.`;
}
