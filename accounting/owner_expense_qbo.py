#!/usr/bin/env python3
"""Post an owner-paid business expense as a balanced QBO journal entry."""
...
def request_id_for_receipt(receipt_id: str) -> str:
    """Return a stable, non-PII QBO request id for one receipt."""
    digest = hashlib.sha256(f"socialsol|owner-expense|{receipt_id}".encode()).hexdigest()[:36]
    return f"ss-oe-{digest}"[:50]
...
def main(argv=None) -> Dict:
    ...
    expense_account_id = require(args.expense_account_id, '--expense-account-id')
    ...
    request_id = request_id_for_receipt(receipt_id)
    ...
    if args.verify_only:
        ...
        entry = client.read_entity('journalentry', qbo_id)
        verified = verify_journal_entry(entry, **expected)
        return {
            'status': 'VERIFIED',
    ...
    created = client.create_journal_entry(journal_entry, request_id=request_id)
    qbo_id = require(created.get('Id'), 'QBO JournalEntry Id')
    entry = client.read_entity('journalentry', qbo_id)
    verified = verify_journal_entry(entry, qbo_id=qbo_id, **{
