// crm/public/lp/px.js — landing-page behavior tracker. Vanilla JS, no deps.
// Renamed from tracker.js to avoid ad-blocker filter-list matches on 'tracker'.
//
...
(function () {
  ...
  // Do Not Track: pageview, wa_click (conversion), and cta_view (CTA visibility)
  // always fire — these are first-party conversion analytics, not behavioral tracking.
  // Facebook's in-app browser sets DNT=1; suppressing conversion events for DNT users
  // was silently breaking attribution for ~90% of paid traffic.
  // DNT suppresses only optional behavioral events (scroll, click, heartbeat).
  ...
  //     ALWAYS active (even for DNT) — this is first-party conversion tracking. ---
  document.addEventListener('click', function (e) {
    var a = e.target && e.target.closest && e.target.closest('a.wa-cta');
    if (!a) return;
    appendWaRef(a);
    var label = a.getAttribute('data-event') || a.getAttribute('data-cta') || 'whatsapp_click';
  ...
  //     ALWAYS active (even for DNT) — first-party CTA visibility is not behavioral tracking.
  //     Also auto-observes .wa-cta elements (not just [data-cta]) so pages without
  //     data-cta attributes still record reached_cta.
  if ('IntersectionObserver' in window) {
    ...
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) {
        if (en.isIntersecting && !seen.has(en.target)) {
          ...
          var el = en.target;
          setTimeout(function () {
            if (el.isConnected) send('cta_view', 'cta:' + (el.getAttribute('data-cta') || el.getAttribute('data-event') || el.id || ''));
          }, 600);
        }
      });
    }, { threshold: 0.6 });
    ...
    var wire = function () {
      document.querySelectorAll('[data-cta]').forEach(observe);
      // Also observe .wa-cta elements that may lack data-cta
      document.querySelectorAll('.wa-cta').forEach(observe);
    };
  ...
  // --- DNT: skip optional behavioral tracking ---
  if (dnt) {
    // Override LPDS_TRACK to no-op for behavioral events only;
    // wa_click and cta_view handlers above are already registered.
    var origSend = send;
    window.LPDS_TRACK = function (kind) {
      if (kind === 'wa_click' || kind === 'cta_view') return origSend.apply(null, arguments);
      // suppress scroll, click, heartbeat, etc.
    };
    return;
  }
  ...
  // (wa_click handler moved above DNT gate — always active)
  ...
  // (CTA observer moved above DNT gate — always active)
