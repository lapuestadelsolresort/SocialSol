...
5. The daily `paulina.prepare_daily` graph records analysis, capacity,
   research, campaign attachment, verification, composition, readback, and its
   Slack summary as separate durable steps. Its one Slack summary without user `@mentions`
   also aggregates every CRM-verified `paulina.daily` result since the prior
   completed summary; the five-minute dispatch graph does not post individual
   success messages. `scripts/engagement-analysis.js` uses the canonical report
   and maintains an idempotent private iteration-state entry for each workflow
   run. Preparation is skipped on weekends, but the send digest still posts if
